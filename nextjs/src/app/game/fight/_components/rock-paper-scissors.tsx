...
export function RockPaperScissorsGame() {
  const loading = useStore(
    (state) => state.game.mutable.gameSpecific === undefined,
  );
  if (loading) return <GameContentLoading />;
...
function ViewContainer() {
  const view = useStore(
    (state) => state.game.mutable.gameSpecific?.logic.showView,
  );
  ...
  switch (view) {
    ...
    case "waiting-for-other-player-choose":
  ...
  return (
    ...
    >
      <div className="flex w-full justify-between">
        <div>{result?.yourId}</div>
        <div>
          {score?.yourScore} - {score?.opponentScore}
        </div>
        <div>{result?.opponentId}</div>
      </div>
      <div className="flex w-full justify-between">
        <div>{result?.youChoose}</div>
        <div>{result?.opponentChoose}</div>
      </div>
    </GameCard>
  );
}
